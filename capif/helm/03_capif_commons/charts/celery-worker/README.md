...
A Helm chart for Kubernetes of celery-worker
...
| image.repository | string | `"celery-worker"` |  |
| image.tag | string | `""` |  |
...
| volumeMounts[0].name | string | `"celery-worker"` |  |
...
| volumes[0].configMap.name | string | `"celery-worker-configmap"` |  |
| volumes[0].name | string | `"celery-worker"` |  |
